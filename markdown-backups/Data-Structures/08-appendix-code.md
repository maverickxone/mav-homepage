...
chapter: 8
readTime: 25
description: "一份可直接编译运行的 C 代码合订：顺序表、单链表、栈、循环队列、二叉树、BST、KMP、七大排序，每段都配一个 test 函数。"
---
...
定长数组 + `length` 计数。插入要**从后往前**挪，删除要**从前往后**挪，平均移动约一半元素，所以增删是 O(n)、随机访问是 O(1)。
...
带头结点能让插入/删除「首元结点」和插入/删除「中间结点」写成同一套代码。注意两个命门：插入是**先接后断**，删除要**记得 `free`**。反转用三指针滚动。
...
复用上面的 `BiTNode`/`newNode`。中序遍历 BST 必然升序。删除分三种情况，最难的是「左右孩子都在」——用**中序后继**（右子树最小值）替换，再去右子树删掉那个后继。
...
KMP 的精髓是 `next` 数组：失配时模式串 `j` 回退、主串 `i` **永不回退**。`getNext` 自己和自己做匹配求出 `next`，这是手算题和代码题都爱考的点。
...
直接插入、希尔、冒泡、快排、简单选择、堆排、归并一次性放在一起对照。记住稳定性与复杂度：快排/堆排/希尔/简单选择**不稳定**，平均最快的是快排，最坏 O(n²) 的是快排和冒泡/插入。
