---
title: "第二章：计算的大脑——CPU与GPU"
chapter: 4
readTime: 45
description: "讲解 CPU、核心、线程、主频、指令集、GPU 并行计算，以及 CPU 与 GPU 的分工逻辑。"
---
...
但这里要注意：一次时钟周期不等于执行一条指令。现代 CPU 通过各种优化，可以在一个时钟周期内并发处理多条指令（这叫**流水线**和**超标量**，我们不深入展开）。但粗略来说，**主频越高，CPU 处理单个任务的速度越快**。
...
多核心对于**多任务**和**可以并行化的程序**非常有用：
- 你一边下载文件，一边看视频，一边写文档——这三件事可以分别跑在不同核心上
- 编译代码（把你写的程序翻译成机器能执行的二进制）是典型的可以并行化的任务，16 核编译比 4 核快得多
...
现代 CPU 普遍支持**超线程技术（Hyper-Threading）**，让一个物理核心可以伪装成两个逻辑处理器。
...
你注意到了吗——它有两种核心：**性能核（P-Core）** 和 **能效核（E-Core）**。这是 Intel 从 12 代开始引入的**大小核架构**（类似于手机 SoC 的设计思路）：

- **性能核**：主频高、单核性能强，处理需要快速响应的任务（游戏主线程、前台应用）
- **能效核**：主频较低、功耗小，处理后台任务（系统服务、下载、同步）
...
这个循环每个时钟周期都在发生，现代 CPU 还有**流水线**（把这四步重叠进行，类似工厂流水线）和**乱序执行**（预测后续指令，提前开始执行）等技术加速这个过程。
...
在取指阶段，CPU 还不知道判断结果——它要等「判断余额」这条指令执行完才知道下一步跳到哪。如果 CPU 什么都不做，就要把流水线后续的位置全部空着，等一个结果———这叫**流水线停顿（Pipeline Stall）**。

对于一个 15 级流水线的 CPU，每次分支预测失败，就要浪费最多 15 个时钟周期——在 5 GHz 的 CPU 上，这是 3 纳秒的损失。程序里平均每 5-7 条指令就有一条分支，这个代价不可忽视。

#### 分支预测：大厨的直觉

解决方案是**分支预测（Branch Prediction）**：CPU 不等结果，直接预测跳转方向，提前把预测路径上的指令塞进流水线继续执行。

- 如果预测对了：一切正常，流水线未中断，性能无损失
- 如果预测错了：清空流水线中错误路径上的指令（**流水线冲刷，Pipeline Flush**），从正确路径重新开始——浪费了那么多周期

现代 CPU 的分支预测精度极高，通常在 **95% 到 99%** 之间。Intel 和 AMD 的分支预测器是非常复杂的硬件学习系统，能识别循环次数、历史跳转模式，甚至利用神经网络思路（感知机分支预测器）。

这件事有一个有趣的实际意义：你写的代码的**数据分布**，会影响CPU的性能。有一个著名的例子：对一个随机数组做判断，比对一个排好序的数组做同样的判断慢数倍——因为乱序数组的分支预测率更低。2012 年 Stack Overflow 有一个讨论这件事的回答，至今仍是该网站历史上点赞最多的答案之一，你可以搜索「Why is processing a sorted array faster than an unsorted array」找到它。
...
乱序执行和分支预测带来了性能提升，但 2018 年，研究人员发现这两种技术也带来了一个严重的安全漏洞——**幽灵（Spectre）** 和 **熔断（Meltdown）**。
...
**❌ 核心数越多，电脑越快**
✅ 取决于任务类型。单线程性能强的 4 核 CPU，在游戏中可能比 16 核但频率低的 CPU 表现更好。"快"是相对于具体任务而言的。

**❌ 主频 3.5 GHz 比 2.5 GHz 快 40%**
✅ 不同架构之间主频没有可比性。Apple M3 的 4 GHz 和 Intel 的 4 GHz 是完全不同的东西——苹果的 IPC（每时钟周期执行的指令数）更高，实际性能远不止靠主频来定。

**❌ 睿频就是正常运行频率**
✅ 睿频（Turbo Boost）是短时峰值频率，只在有散热余量时触发，且只持续几秒到几十秒。长时间负载下，CPU 会因温度限制降频到"长期功耗墙"对应的频率。这就是为什么某些笔记本标称 5 GHz，实测持续性能却很一般。
...
- ✓ CPU 是计算机的大脑，负责执行所有指令，特点是通用、精确、串行
- ✓ 主频决定单核速度，核心数决定并行能力，二者服务于不同场景
- ✓ 超线程让一个核心对外表现为两个逻辑处理器，提升利用率
- ✓ 大小核架构（Intel 12代+）是性能与能效的平衡设计
- ✓ 睿频只是短时峰值，散热是决定实际性能的重要因素
...
为什么？因为它们说的是不同的"方言"——这就是**指令集架构（ISA，Instruction Set Architecture）**。

---

### 什么是指令集

CPU 本质上是一堆逻辑电路，它能直接理解的是**机器码**——一串二进制数，比如 `10110000 01100001`。这串二进制对 CPU 来说是一条具体的指令："把值 97 存入某个寄存器"。
...
特点：
- **CISC（复杂指令集）**：指令数量多，单条指令可以完成复杂操作
- 指令长度可变（1-15 字节不等），解码复杂
- 历史包袱重，但生态极其庞大
- 功耗相对较高
...
特点：
- **RISC（精简指令集）**：指令数量少，每条指令做的事情简单
- 指令长度固定（AArch64 模式下每条指令固定 4 字节），解码简单，功耗低
- 授权模式（ARM 公司卖 IP，其他公司设计芯片），生态碎片化但覆盖极广
- 功耗效率极高
...
对于编译型语言（C、C++、Rust、Go），这个翻译在你运行程序之前完成，产生特定架构的二进制文件——这就是为什么你下载软件时有时需要选 `x86_64` 或 `arm64`。
...
现代编译器（如 GCC、Clang/LLVM）是极其复杂的软件，它们不仅翻译语言，还做大量**优化**：

- **内联展开（Inlining）**：把小函数的代码直接嵌入调用处，消除函数调用开销
- **循环展开（Loop Unrolling）**：把循环体复制几次，减少循环控制的开销，方便流水线
- **向量化（Vectorization / SIMD）**：把多个数据的相同操作合并成一条 SIMD 指令（如 AVX-512，一条指令处理 16 个 32 位浮点数），大幅提升数值计算性能
- **寄存器分配**：把变量尽量放在寄存器里，减少内存访问
...
### ❌ 常见误区澄清 {#03-cpu-gpu--常见误区澄清-2}

**❌ ARM 指令集是苹果发明的**
✅ ARM 指令集由 ARM Holdings 公司（现属于软银）所有。苹果只是 ARM 的大客户之一，授权了 ARM 指令集设计自己的微架构（如 Firestorm、Everest 等核心）。就像你可以用汉语写出不同风格的文章，苹果用 ARM"语言"写出了自己独特的"文章"。

**❌ 64 位比 32 位就是更快**
✅ 64 位主要意义是：能寻址更大的内存空间（32 位最多 4GB，64 位理论上可达 16EB）。对于计算速度本身，64 位不一定更快，甚至有些情况下 32 位程序因为数据量更小反而更快。关键是它打破了 4GB 内存上限的枷锁。
...
```
输出类似 `Intel64 Family 6 Model 154 Stepping 3` 说明是 x86-64。
如果看到 `ARM64`，你用的是 ARM Windows 设备（如 Surface Pro X）。
...
还可以更进一步：在 macOS 上打开"系统报告"（关于本机 → 更多信息 → 系统报告），在"软件"→"应用程序"列表里，可以看到每个 App 的"种类"：`Intel`、`Apple`（原生 ARM）或 `Universal`（两者都有）。

---

### ✓ 本节要点 {#03-cpu-gpu--本节要点-2}

- ✓ 指令集是 CPU 能直接理解的"语言"，不同指令集的程序不能直接互换运行
- ✓ x86-64（CISC）是桌面/服务器的主流，ARM（RISC）统治移动端并正在进攻桌面端
- ✓ RISC 和 CISC 的核心区别是指令的复杂程度，现代实现中差距已经很小
- ✓ 苹果 M 系列是 ARM 授权 + 自研微架构，不等于"苹果架构"
- ✓ RISC-V 是开源指令集，代表了未来可能的第三条路
...
你看到的游戏画面，是由数百万个像素组成的。以 4K 分辨率（3840×2160）为例，一帧有 **8294400 个像素**。游戏跑在 60 帧每秒，意味着每秒要计算 **约 5 亿个像素**的颜色值。

每个像素的颜色，取决于：光源方向、物体材质、阴影、反射、环境光遮蔽……一个像素的着色计算可能涉及几十到几百次浮点运算。

关键在于：**这些像素的计算彼此之间完全独立**。第 100 万号像素的颜色，和第 200 万号像素的颜色没有任何数据依赖关系——它们可以**同时计算**。
...
回到餐厅的比喻。如果说 CPU 是精通所有厨艺、能应对任何复杂订单的大厨，那 GPU 就是一支**拥有数千名流水线工人的工厂**——每个工人只会做一件简单的事（比如：计算一个像素的颜色），但他们可以**同时出发**。
...
以 RTX 4090 为例：
- 128 个 SM
- 每个 SM 里有 128 个 CUDA Core
- 合计 16384 个 CUDA Core
- 显存带宽：**1008 GB/s**（相比之下，DDR5 内存带宽约 50-100 GB/s）
...
一帧画面是怎么从游戏引擎里的三维场景，变成你屏幕上的像素的？这个过程叫**渲染管线（Rendering Pipeline）**，大体上分为几个阶段：
...
这个管线里，**顶点着色**和**片段着色**（也叫像素着色）是计算最密集的两步，也是那些「着色器程序（Shader）」运行的地方。着色器是跑在 GPU 上的小程序，由游戏开发者用 HLSL（Direct3D）或 GLSL（OpenGL）编写。
...
即便如此，全分辨率的完整光追仍然开销极大，实际游戏里通常只对特定效果（反射、阴影、全局光照）开启光追，其余部分仍用传统光栅化——这叫**混合渲染（Hybrid Rendering）**。
...
这 43 亿次运算之间**完全没有数据依赖**——第 $(i, j)$ 个输出元素的计算，和第 $(i', j')$ 个输出元素的计算毫无关系，可以完全并行。GPU 的数千个核心，就是为这种任务而生的。
...
### ❌ 常见误区澄清 {#03-cpu-gpu--常见误区澄清-3}

**❌ 显存越大，显卡越好**
✅ 显存大小只是一个维度。一张 16GB 的低端显卡，在渲染性能上远不如 8GB 的高端显卡。显存更像是"工厂的货架大小"，决定一次能放多少材料，但工人的速度（算力、带宽）才是核心。在 AI 推理场景下，显存大小反而更重要，因为模型权重需要全部装进显存——这个场景下"货架大小"是硬约束。

**❌ 独立显卡总比集成显卡好**
✅ 集成显卡（核显）近年来进步很快。苹果 M3 的 GPU 部分，在功耗控制下能胜过不少入门级独显，且共享内存带宽更高（因为 CPU 和 GPU 在同一个芯片上，访问同一块内存）。对于不玩 3A 游戏、不跑深度学习的用户，M 系列的集成 GPU 完全够用。

**❌ GPU 可以完全替代 CPU**
✅ GPU 擅长大规模并行、计算规则简单的任务。操作系统调度、网络协议栈、数据库查询、大多数业务逻辑——这些充满条件分支、顺序依赖的任务，GPU 处理起来很吃力。CPU 和 GPU 是互补关系，不是替代关系。
...
### ✓ 本节要点 {#03-cpu-gpu--本节要点-3}

- ✓ GPU 的核心价值是大规模并行计算，擅长处理大量独立的简单运算
- ✓ 渲染画面、训练神经网络都是典型的并行计算任务，天然适合 GPU
- ✓ GPU 拥有数千到数万个简单核心，配合极高带宽的专用显存工作
- ✓ CUDA 让 GPU 从专用图形硬件变成了通用并行计算平台
- ✓ 显存大小 ≠ 显卡性能，但在 AI 推理部署中是关键约束
...
延迟，是从"发出请求"到"得到结果"的时间。CPU 的每个核心，都配备了大量用于预测、缓存、乱序执行的电路，目的只有一个：让**这一个**任务尽快完成。

想象一个高级客服：她有一个请求，立刻调用所有资源来解决它——查数据库、调系统、和后台沟通——总之以最快速度给你答复。她一次只服务一个人，但这个人的等待时间极短。

**GPU 的设计目标：最大化吞吐量（Throughput）**

吞吐量，是单位时间内处理多少任务的总量。GPU 不在乎某一个任务多快完成，它在乎的是**所有任务**在单位时间内完成多少。
...
现代计算机不是 CPU 干全部活，也不是 GPU 干全部活——而是**异构协同**：
...
在这个流程里，CPU 和 GPU 通常是**流水线式并行**的：当 GPU 在渲染第 N 帧时，CPU 已经在处理第 N+1 帧的游戏逻辑了。
...
**训练阶段：**
- CPU：数据预处理（读文件、增强、打包成 Batch），把数据送给 GPU
- GPU：执行前向传播（矩阵乘法、激活函数），反向传播（梯度计算），参数更新

**推理阶段（模型上线服务）：**
- CPU：接收用户请求，管理并发，路由任务
- GPU：批量执行模型前向传播，返回结果

这就是为什么大型 AI 服务器不只是堆 GPU——它们同样需要足够强大的 CPU 来"喂饱"GPU，否则 GPU 会因为等待数据而空转（这叫 **I/O 瓶颈**或 **CPU 瓶颈**）。
...
这些 NPU 是做什么用的？主要是：
- 实时人脸识别解锁（每秒处理摄像头帧）
- 相机的计算摄影（夜景融合、人像虚化的分割）
- 语音唤醒（在极低功耗下持续监听唤醒词）
- 手机端侧 AI 推理（如苹果的 Apple Intelligence）
...
- GPU 是通用并行处理器，同时支持渲染和 GPGPU（通用 GPU 计算）
- TPU 完全不支持渲染，只为神经网络的矩阵运算优化，控制逻辑极简
...
训练大模型需要大量显存。GPT-3 的参数量是 1750 亿，以 FP16（16 位浮点，每个参数 2 字节）存储需要 350 GB 显存，远超单张 H100 的 80 GB。解决方案是**多卡并行训练**，让模型分布在多张 GPU 上。
...
苹果 M 系列芯片带来了一个有趣的变化：CPU 和 GPU 不再是两块独立的芯片，而是集成在同一块 SoC（System on Chip，片上系统）上，并且**共享同一块内存（统一内存，Unified Memory）**。
...
- 实时三维渲染
- 深度学习训练与推理（矩阵乘法密集）
- 密码学（暴力破解哈希，这也是挖矿用 GPU 的原因）
- 图像/视频批量处理
- 科学计算（流体模拟、分子动力学、气候模型）
- 信号处理（FFT 大规模并行化）

**边界模糊的地方：**
一些任务原本在 CPU 上跑，因为规模扩大，开始迁移到 GPU。比如数据库查询（GPU 数据库，如 RAPIDS），比如推荐系统的特征计算。这个边界还在移动，是个活跃的研究领域。

---

### ❌ 常见误区澄清 {#03-cpu-gpu--常见误区澄清-4}

**❌ 游戏的 FPS 越高，是 GPU 越好的功劳**
✅ 不完全对。在高分辨率（4K）下，渲染的压力主要在 GPU，此时 GPU 性能是瓶颈；但在低分辨率（1080P）+高帧率场景下，GPU 完成渲染很快，等待 CPU 提交下一帧的命令成了瓶颈——这叫 **CPU 瓶颈**。这也是为什么追求 240Hz 的电竞选手即使用了顶级显卡，也需要搭配强力 CPU。

**❌ 跑 AI 模型只需要 GPU，CPU 无所谓**
✅ CPU 决定了数据预处理的速度和任务调度能力。一块顶级 GPU 配上羸弱的 CPU，会出现"GPU 空转等数据"的情况，整体训练速度反而比配置均衡的机器慢。
...
然后：
4. 把游戏画质从最高调到最低（或把视频从 4K 换成 1080P）
5. 再次观察 CPU 和 GPU 负载
...
### ✓ 本节要点 {#03-cpu-gpu--本节要点-4}

- ✓ CPU 针对延迟优化（让单个任务尽快完成），GPU 针对吞吐量优化（让总量尽可能多）
- ✓ 现代计算机是异构系统，CPU 和 GPU 协同工作，各司其职
- ✓ CPU 瓶颈和 GPU 瓶颈是两种不同的性能限制，解决方案完全不同
- ✓ 苹果统一内存架构消除了 CPU-GPU 之间的数据复制开销，在特定场景下极具优势
- ✓ AI 计算让 GPU 的重要性从"游戏显卡"上升到"算力基础设施"
...
第一章里我们建立了存储金字塔：寄存器、缓存、内存、硬盘。这一章揭示了**谁在使用这张金字塔**——CPU 的寄存器和缓存是第一章概念的具体化，GPU 则有自己独立的显存体系，但本质上也遵循"离执行单元越近，速度越快、容量越小"的规律。
...
具体数字：
- 1971 年 Intel 4004：约 2300 个晶体管，10 微米制程
- 2000 年 Intel Pentium 4：约 4200 万个晶体管，180 纳米制程
- 2024 年 Apple M4：约 280 亿个晶体管，3 纳米制程
...
**视频：**
- 《How a CPU Works》— In One Lesson（YouTube）⭐ 入门，13分钟，用直观动画演示 CPU 取指-执行循环
- 《But How Do Transistors Work?》— Lesics（YouTube）⭐ 入门，从物理层面理解 CPU 里的基本单元
- 《GPU Architecture》— NVIDIA Deep Learning Institute 公开课（YouTube）⭐⭐ 进阶，NVIDIA 官方出品，讲 CUDA 编程模型和 GPU 架构
- 《Inside the Apple M1》— Asahi Linux Blog（文章）⭐⭐⭐ 深入，逆向工程角度分析 M1 微架构，技术密度极高

**文章：**
- Agner Fog《Instruction tables》⭐⭐⭐ 深入，统计各 x86 指令的延迟和吞吐量，用于底层性能分析（偏研究向）
- Anandtech 的芯片评测文章（如《Apple M3 Review》）⭐⭐ 进阶，严谨的工程师视角，有大量测试数据和架构分析
- Wikipedia：《Instruction set architecture》⭐⭐ 进阶，系统梳理 ISA 概念和各主流指令集

**中文资源：**
- 《深入理解计算机系统（CSAPP）》第三、五章 ⭐⭐⭐ 深入，中科大计算机系的必读书，这两章讲汇编和性能优化，是理解 CPU 指令集的经典材料
- Bilibili 搜索"CPU是怎么工作的 硬核科普"——有若干质量不错的中文视频 ⭐ 入门
