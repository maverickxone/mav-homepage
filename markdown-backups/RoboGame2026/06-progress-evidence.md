---
title: "项目进度总览：哪些已经做好，哪些还在路上"
chapter: 7
readTime: 48
description: "沿着代码仓库的修改历史和当前源码，看看项目已经完成了什么、还有什么在路上，学会区分'代码写好了'和'车上验证过了'的区别。"
...
:::callout 💡 七道门——嵌入式功能从"想到"到"做到"要过几关？
...
5. **烧录阶段**：把固件写到 STM32 的 Flash 里（**烧录**就是把程序写进芯片，就像往 U 盘里拷文件）
...
:::callout 💡 零基础小课堂：AXF / HEX / BIN 是什么？
...
| 2026-06-09 | `7eae974`、`de7cb00`、`6b23148` | 调整 ICM20948、精简初始化、清理冲突依赖 | 修修补补：传感器初始化和代码冲突问题 |
| 2026-06-11 | `6e17464` | 加入 FlyMcu 烧录程序 | 准备好了"把程序写进芯片"的工具 |
| 2026-06-12 | `6743876` | 更新协议与生成的消息编解码代码 | 协议"合同"更新了一版 |
| 2026-06-13 | `0191992`、`fc3ef5d` | 接入部分运动命令，通信口切到 USART1 | 第一组运动命令能用了，串口换到 USART1 |
...
:::callout 💡 初学者常见疑问：为什么 6 月 13 日之后就没有新提交了？
...
1. **`PING`**
   这是通信链路最基础的"心跳"命令。上位机发一个 `PING`，下位机回复 ACK OK，说明"我能收到你、我也能回你"。调试时第一件事往往就是反复发 `PING`，确认串口接线、波特率、帧格式都没问题。如果连 `PING` 都不通，后面任何运动命令都不用试。
...
3. **`TEST_COMMAND`**
   这是一条专门给调试用的"回音壁"命令。上位机发一个 32 位整数，下位机原样把它作为 telemetry 送回。你可以用它来验证：
   - 发送的帧格式是否正确；
   - 参数能不能被正确解析；
   - 下位机的响应链路是否工作。
...
5. **`START_ROTATION`**
   和 `START_MOTION` 类似，只不过它输出固定 PWM 让小车原地旋转。顺时针或逆时针由方向参数决定。
...
:::callout-tip 💡 `NOT_IMPLEMENTED` 不是 bug，是诚实的进度标记
...
:::callout 💡 初学者常见疑问：为什么 MOTION 里已经写了 START_MOTION，却说闭环没接好？
...
`USER/WHEELTEC.uvprojx` 是 Keil 的**工程文件**（记录了哪些源代码参与编译），目标名是 `Mec_Car`，芯片为 STM32F407VE。工程文件已经包含 APP、协议、UART 和运动控制的 `.c` 文件，说明新的主链路进入了构建清单。
...
2. **烧录是否成功？**
   仓库里有 `FlyMcu2188.exe`，说明作者已经为烧录（把程序写进芯片，就像往 U 盘里拷文件）准备了一个工具入口。但"有工具"不等于"已经烧过并且成功"。
...
:::callout-warn ⚠️ 文档和代码不一致时，以代码为准
