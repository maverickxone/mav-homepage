...
- [第一章：为什么会有Rust？——从C的原罪说起](#chapter-一)
  - [1.1 C统治世界六十年靠的是什么](#1-1)
    - [1.1.1 一门语言的长寿秘诀](#1-1-1)
    - [1.1.2 C的设计哲学：相信程序员](#1-1-2)
    - [1.1.3 六十年积累的技术债](#1-1-3)
  - [1.2 内存安全：那70%的漏洞从哪来](#1-2)
    - [1.2.1 内存管理的两种思路](#1-2-1)
    - [1.2.2 为什么sanitizer不够用](#1-2-2)
  - [1.3 C++试图修补，但越补越复杂](#1-3)
    - [1.3.1 C++的历史使命](#1-3-1)
    - [1.3.2 智能指针：一个局部解](#1-3-2)
    - [1.3.3 C++委员会：百年老店的治理困境](#1-3-3)
    - [1.3.4 C++的数据竞争问题](#1-3-4)
  - [1.4 Mozilla的工程师为什么要重新造轮子](#1-4)
    - [1.4.1 一个具体的工程问题](#1-4-1)
    - [1.4.2 Servo：那个"实验引擎"](#1-4-2)
    - [1.4.3 为什么不直接用Go？](#1-4-3)
  - [1.5 Rust的诞生：一个浏览器引擎的副产品](#1-5)
    - [1.5.1 从个人项目到正式语言](#1-5-1)
    - [1.5.2 Rust团队的核心决策](#1-5-2)
    - [1.5.3 Rust今天是什么](#1-5-3)
- [第二章：Rust到底在解决什么问题？——它的核心哲学](#chapter-二)
  - [2.1 "零成本抽象"是什么意思](#2-1)
    - [2.1.1 抽象必然有代价吗](#2-1-1)
    - [2.1.2 用迭代器做一个具体的例子](#2-1-2)
    - [2.1.3 和Python的对比：不同的取舍](#2-1-3)
  - [2.2 所有权系统：不靠GC，也不靠程序员自律](#2-2)
    - [2.2.1 所有权：一个简单的规则](#2-2-1)
    - [2.2.2 作用域决定生命周期](#2-2-2)
    - [2.2.3 移动（Move）：转移所有权](#2-2-3)
    - [2.2.4 这和C的free有什么本质不同](#2-2-4)
  - [2.3 借用检查器：编译器当你的保姆](#2-3)
    - [2.3.1 借用（Borrow）：不转移所有权地使用值](#2-3-1)
    - [2.3.2 借用的规则：为什么这么严格](#2-3-2)
    - [2.3.3 借用检查器：让人又爱又恨](#2-3-3)
    - [2.3.4 借用检查器的进化：NLL](#2-3-4)
  - [2.4 和C对比：同样快，但更难写错](#2-4)
    - [2.4.1 性能：基本相当](#2-4-1)
    - [2.4.2 内存控制：同样精细](#2-4-2)
    - [2.4.3 写错的难度：差异显著](#2-4-3)
    - [2.4.4 unsafe：Rust的逃生舱](#2-4-4)
  - [2.5 和Python对比：同样现代，但完全不同的取舍](#2-5)
    - [2.5.1 表面相似，本质不同](#2-5-1)
    - [2.5.2 类型系统：静态 vs 动态](#2-5-2)
    - [2.5.3 并发：不同的思路](#2-5-3)
    - [2.5.4 生态和适用场景](#2-5-4)
- [第三章：Rust长什么样？——给C党的语法速览](#chapter-三)
  - [3.1 变量、函数、控制流：和C像不像](#3-1)
    - [3.1.1 变量声明：let和mut](#3-1-1)
    - [3.1.2 基本类型](#3-1-2)
    - [3.1.3 函数](#3-1-3)
    - [3.1.4 控制流：熟悉但有差异](#3-1-4)
  - [3.2 所有权三板斧：Move / Borrow / Lifetime](#3-2)
    - [3.2.1 Move：转移所有权](#3-2-1)
    - [3.2.2 Borrow：引用和借用](#3-2-2)
    - [3.2.3 Lifetime：生命周期注解](#3-2-3)
  - [3.3 枚举和模式匹配：比C的union好在哪](#3-3)
    - [3.3.1 C的union：手动类型标签](#3-3-1)
    - [3.3.2 Rust枚举：类型安全的带数据枚举](#3-3-2)
    - [3.3.3 Option：消灭空指针](#3-3-3)
    - [3.3.4 模式匹配：match不只是switch](#3-3-4)
  - [3.4 错误处理：Result代替异常](#3-4)
    - [3.4.1 三种错误处理思路](#3-4-1)
    - [3.4.2 Result的使用](#3-4-2)
    - [3.4.3 ?运算符：优雅的错误传播](#3-4-3)
    - [3.4.4 panic!：不可恢复的错误](#3-4-4)
  - [3.5 Cargo：Rust的包管理工具](#3-5)
    - [3.5.1 C的痛苦：没有标准包管理](#3-5-1)
    - [3.5.2 Cargo：一站式解决](#3-5-2)
    - [3.5.3 trait：Rust的接口系统](#3-5-3)
- [第四章：Rust在哪里出没？它的边界和地盘](#chapter-四)
  - [4.1 先说结论：Rust不是全能药](#4-1)
  - [4.2 系统编程：Rust的老家](#4-2)
    - [4.2.1 操作系统：从理论到现实](#4-2-1)
    - [4.2.2 驱动程序：另一个雷区](#4-2-2)
    - [4.2.3 嵌入式：STM32上的铁锈味](#4-2-3)
  - [4.3 WebAssembly：前端里的铁锈](#4-3)
    - [4.3.1 WebAssembly是什么](#4-3-1)
    - [4.3.2 为什么Rust和Wasm特别搭](#4-3-2)
    - [4.3.3 真实使用案例](#4-3-3)
  - [4.4 CLI工具：你可能已经在用Rust写的东西](#4-4)
    - [4.4.1 这个赛道Rust已经赢了](#4-4-1)
    - [4.4.2 你可能用过的Rust CLI工具](#4-4-2)
    - [4.4.3 为什么这些工具要用Rust重写](#4-4-3)
  - [4.5 编辑器与基础设施：Zed就是这里的代言人](#4-5)
    - [4.5.1 Zed：你被它的速度惊到的那个编辑器](#4-5-1)
    - [4.5.2 Zed背后的技术选择](#4-5-2)
    - [4.5.3 类似的基础设施项目](#4-5-3)
  - [4.6 Rust不擅长什么：划清边界](#4-6)
    - [4.6.1 快速原型和脚本](#4-6-1)
    - [4.6.2 数据科学和机器学习](#4-6-2)
    - [4.6.3 业务后端和CRUD应用](#4-6-3)
    - [4.6.4 GUI应用](#4-6-4)
- [第五章：世界怎么看Rust？舆论的演变史](#chapter-五)
  - [5.1 这是一部比技术本身更有趣的历史](#5-1)
  - [5.2 2010–2015年：从实验室到1.0](#5-2)
    - [5.2.1 Mozilla的内部项目](#5-2-1)
    - [5.2.2 2015年：1.0发布，承诺稳定性](#5-2-2)
  - [5.3 2016–2019年：小圈子狂热](#5-3)
    - [5.3.1 "最受喜爱语言"的连续登顶](#5-3-1)
    - [5.3.2 Rustacean文化的形成](#5-3-2)
    - [5.3.3 第一批重量级采用者](#5-3-3)
  - [5.4 2020年：Linux内核里的那场争论](#5-4)
    - [5.4.1 "Rust进Linux内核"的提案](#5-4-1)
    - [5.4.2 反对的声音](#5-4-2)
    - [5.4.3 Linus的态度转变](#5-4-3)
  - [5.5 2022年：巨头集体站台](#5-5)
    - [5.5.1 微软](#5-5-1)
    - [5.5.2 Google](#5-5-2)
    - [5.5.3 NSA](#5-5-3)
    - [5.5.4 AWS、Meta、Cloudflare](#5-5-4)
  - [5.6 2023–2026年：从"值得尝试"到部分领域的事实标准](#5-6)
    - [5.6.1 白宫的报告](#5-6-1)
    - [5.6.2 Stack Overflow调查：数据的意义](#5-6-2)
    - [5.6.3 "重写成Rust"成为一种文化现象](#5-6-3)
    - [5.6.4 培训和招聘市场的变化](#5-6-4)
  - [5.7 "最受喜爱"背后的深层逻辑](#5-7)
    - [5.7.1 为什么工程师喜欢Rust](#5-7-1)
    - [5.7.2 为什么不喜欢Rust的人不喜欢](#5-7-2)
- [第六章：2026年的Rust现状——它现在在哪里，走向哪里](#chapter-六)
  - [6.1 站在2026年看Rust](#6-1)
  - [6.2 Linux内核：已经落地，正在扩展](#6-2)
    - [6.2.1 从Linux 6.1到现在](#6-2-1)
    - [6.2.2 内核Rust的现状和争议](#6-2-2)
    - [6.2.3 真正的影响：示范效应](#6-2-3)
  - [6.3 Windows：微软的Rust化进程](#6-3)
    - [6.3.1 已经完成的部分](#6-3-1)
    - [6.3.2 Rust for Windows的开发者工具](#6-3-2)
    - [6.3.3 Azure基础设施](#6-3-3)
  - [6.4 Android和Chrome：Google的Rust版图](#6-4)
    - [6.4.1 Android里的150万行Rust代码](#6-4-1)
    - [6.4.2 Chromium里的Rust](#6-4-2)
  - [6.5 2026年的明星项目](#6-5)
    - [6.5.1 Zed：编辑器领域的标杆](#6-5-1)
    - [6.5.2 uv：Python工具链的颠覆者](#6-5-2)
    - [6.5.3 Pingora：Cloudflare的NGINX替代](#6-5-3)
    - [6.5.4 Deno和Bun：JS运行时的Rust渗透](#6-5-4)
    - [6.5.5 Rust在AI基础设施里](#6-5-5)
  - [6.6 Rust基金会和社区生态](#6-6)
    - [6.6.1 Rust基金会](#6-6-1)
    - [6.6.2 crates.io生态](#6-6-2)
    - [6.6.3 Rust的版次（Edition）系统](#6-6-3)
  - [6.7 下一个十年：会成为下一个C吗](#6-7)
    - [6.7.1 "下一个C"是一个很高的标准](#6-7-1)
    - [6.7.2 乐观的理由](#6-7-2)
    - [6.7.3 悲观的理由](#6-7-3)
    - [6.7.4 我的判断](#6-7-4)
- [尾声：我要不要学Rust？——一个微电子+AI方向的人的视角](#epilogue)
  - [结语.1 先说结论](#epilogue-1)
  - [结语.2 嵌入式场景：Rust对你意味着什么](#epilogue-2)
    - [结语.2.1 STM32有Rust支持吗](#epilogue-2-1)
    - [结语.2.2 RISC-V上的Rust](#epilogue-2-2)
  - [结语.3 AI基础设施：Rust在你的方向上扮演什么角色](#epilogue-3)
    - [结语.3.1 直接角色有限](#epilogue-3-1)
    - [结语.3.2 AI基础设施的底层趋势](#epilogue-3-2)
  - [结语.4 学习成本评估：有C基础需要多久入门](#epilogue-4)
    - [结语.4.1 你的优势](#epilogue-4-1)
    - [结语.4.2 你的劣势](#epilogue-4-2)
    - [结语.4.3 时间估算](#epilogue-4-3)
    - [结语.4.4 最好的学习路径](#epilogue-4-4)
  - [结语.5 最终结论：你和Rust的关系](#epilogue-5)
