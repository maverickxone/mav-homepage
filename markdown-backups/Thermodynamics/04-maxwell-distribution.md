...
chapter: 4
readTime: 60
description: "平衡态下分子速率服从 Maxwell 分布——从这条曲线读出三个特征速率，用高斯积分把平均量算出来，再引出能量均分定理与玻尔兹曼因子。含五张配图与例题精讲。"
---
...
> [图片 04-03：速度空间中的球壳。速率在 v 到 v+dv 之间的分子，落在以原点为心、半径 v、厚 dv 的球壳内，其体积正比于 4πv²dv——这就是 Maxwell 分布里那个 v² 因子的几何来源。]
...
> [图片 04-01：Maxwell 速率分布曲线与三个特征速率。最概然速率 vₚ 在峰顶，平均速率 v̄ 稍大，方均根速率 v_rms 最大；三者之比恒为 vₚ:v̄:v_rms = 1:1.13:1.22。]
...
> [图片 04-02：温度对速率分布的影响。温度升高（T₁<T₂<T₃），峰值右移、变矮、变宽；三条曲线下方面积都恒为 1。]
...
> [图片 04-04：自由度示意。单原子分子只有 3 个平动自由度（i=3）；双原子分子加 2 个转动自由度（i=5）；非线性多原子分子有 3 个转动自由度（i=6）。能量均分定理给每个平方项分 ½kT。]
...
> [图片 04-05：Stern 实验示意。中心源射出的原子束经狭缝准直，打在旋转圆筒的内壁；快原子和慢原子因飞行时间不同而落在不同位置，由沉积层厚度反推速率分布。]
